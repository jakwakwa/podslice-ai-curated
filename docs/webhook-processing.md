...
This service ingests signed webhooks from Paddle (`app/api/webhook/route.ts`) and hands the payload to `ProcessWebhook`. Critical flows for `subscription.updated` and `payment.failed` were hardened to avoid missing lifecycle changes.
...
3. **Idempotent persistence**  
   ...
   - Payment failures always update the subscription’s status to `past_due` (to capture individual attempts and retry windows).
...
5. **Notifications**  
   In-app notifications for Paddle billing events have been removed. We rely on Paddle's native email notifications for subscription updates, payment failures, and renewals. Ensure these are enabled in the Paddle Dashboard.
