...
1. **subscription.created / subscription.updated**
    *   Extract subscription data (status, price\_id, billing period)
    *   Map price\_id to plan\_type using `priceIdToPlanType()`
...
*   **Prisma 6.16.2**
    ...
    *   Snake\_case field naming with `@map` directives
...
*   **Subscription Lifecycle**: States a subscription moves through (trialing → active → past\_due → canceled)
