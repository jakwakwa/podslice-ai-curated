...
export async function POST(request: Request) {
	try {
		...
		if (userProfiles.length === 0) {
			return NextResponse.json({ message: "No users found for this bundle" }, { status: 404 });
		}
		...
		for (const profile of userProfiles) {
			try {
				...
				const baseUrl = process.env.NODE_ENV === "development" ? "https://podslice-ai-synthesis.vercel.app" : process.env.NEXT_PUBLIC_APP_URL;
