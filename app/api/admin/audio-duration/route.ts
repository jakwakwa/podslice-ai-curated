...
import {
	updateMissingEpisodeDurations,
	updateMissingUserEpisodeDurations,
} from "@/app/(protected)/admin/audio-duration/duration-extractor";
...
export async function POST(request: Request) {
	try {
		...
		if (type === "user-episodes") {
			...
			return NextResponse.json(
				{ error: "Invalid type. Use 'user-episodes' or 'episodes'" },
				{ status: 400 }
			);
		}
	} catch (error) {
