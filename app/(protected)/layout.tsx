...
function ProtectedLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <SidebarInset>
        <header
          className={`fixed flex ${isMobile ? "h-18" : "h-16"}	 bg-header/30 backdrop-blur-[8px]  shadow-[0_12px_19px_4px_rgba(20,1,10,0.2)] overflow-hidden shrink-0 items-center border-none gap-4 transition-[width,height]  group-has-data-[collapsible=icon]/sidebar-wrapper:h-14 mt-0 max-w-screen justify-between px-4  py-0 overflow-y-scrol  duration-700 ease-in-out z-50 ${state === "expanded" ? "" : "collapsed"}`}
        >
          <div
