...
function ProtectedLayoutInner({ children }: { children: React.ReactNode }) {
	...
	return (
		<>
			...
			<SidebarInset>
				<header
					className={`fixed flex ${isMobile ? "h-18" : "h-16"} bg-cyan-900/90  glass backdrop-blur-2xl   shadow-[0_12px_19px_4px_rgba(20,1,10,0.2)] overflow-hidden shrink-0 items-center border-none gap-4 transition-[width,height]  group-has-data-[collapsible=icon]/sidebar-wrapper:h-14 mt-0 max-w-screen justify-between px-4  py-0 overflow-y-scrol  duration-700 ease-in-out z-50 ${state === "expanded" ? "" : "collapsed"}`}
				>
					<div
					...
					<div className="flex flex-row-reverse items-center gap-5">
						...
						{isMobile && typeof window !== "undefined" && (
							<Drawer
								...
								onOpenChange={setOpenMobileDrawer}
							>
								...
								<DrawerContent className="sm:max-w-screen flex flex-col justify-center items-center bg-cyan-400/35 border-none rounded-t-xl p-0 w-full ">
									<DrawerHeader>
				...
				>
					...
					<div
						className={` md:w-full animated-gradient   mx-0  p-0 flex flex-col my-0 md:flex-row pt-6 md:pb-2 md:pt-16 md:mx-0 pl-0  md:my-2   ${state === "expanded" ? "m-0 md:ml-0 md:p-0  lg:px-2 lg:pb-8 max-w-full" : "pl-12 pr-24 md:ml-0  min-w-screen  bg-header"}`}
					>
						{children}
					</div>
					<Footer />
				...
				{mobileNav.map((item) => (
					<li key={item.title} className=" text-left text-sm w-full ">
						{item.subItems && item.subItems.length > 0 ? (
							...
							<div className=" bg-teal-900/30 rounded-lg flex text-left flex-col items-start justify-start  mx-auto gap-2 font-medium border border-cyan-200/10 shadow-lg shadow-cyan-400/10 text-shadow-cyan-900/10 text-cyan-100/70 max-w-[70%] md:max-w-full  my-0 p-0 ">
									...
									{item.subItems.map((subItem) => (
										<li key={subItem.name} className="text-left text-sm w-full">
											<Link
												...
												className="py-3 bg-white/10 rounded-lg flex text-left flex-row items-center justify-start cursor-pointer gap-2 font-normal shadow-sm text-cyan-100/60 max-w-[60%] px-8 min-w-full my-0"
											>
												{subItem.name}
											</Link>
										</li>
							...
							<Link
								href={item.url}
								onClick={() => setOpenMobileDrawer(false)}
								className="py-3 bg-teal-900/30 rounded-lg flex text-left flex-row items-center justify-start cursor-pointer  my-0 mx-auto gap-2 font-medium border border-cyan-200/10 shadow-lg shadow-cyan-400/10 text-shadow-cyan-900/10  text-cyan-100/70 max-w-[70%] md:max-w-fit pl-8  "
							>
								{item.icon && <item.icon className="size-4 opacity-[0.5]" />}
								{item.title}
							</Link>
