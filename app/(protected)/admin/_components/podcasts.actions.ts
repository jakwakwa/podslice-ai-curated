...
export async function deletePodcastAction(podcastId: string) {
	...
	const podcast: PodcastWithBundles | null = await prisma.podcast.findUnique({
		...
		include: {
			bundle_podcast: {
				include: {
					bundle: { include: { user_curation_profile: { where: { is_active: true } } } },
				},
			},
		},
	});
	...
	const activeUsage = podcast.bundle_podcast.some(
		(bp: PodcastWithBundles["bundle_podcast"][number]) =>
			bp.bundle.user_curation_profile.length > 0
	);
	if (activeUsage)
		throw new Error(
			"Cannot delete podcast; it is used by active user profiles. Consider deactivating instead."
		);
