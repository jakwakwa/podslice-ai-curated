...
describe("episodes API derivation", () => {
	...
	it("returns episodes by podcasts in selected bundle + own profile episodes", async () => {
		...
		await prisma.bundlePodcast.create({
			data: { bundle_id: bundle.bundle_id, podcast_id: pA.podcast_id },
		});

		const profile = await prisma.userCurationProfile.create({
			data: { user_id: user.user_id, name: "Profile" },
		});
		await prisma.userCurationProfile.update({
			where: { profile_id: profile.profile_id },
			data: { selected_bundle_id: bundle.bundle_id },
		});

		const [epA, epB, epMine] = await Promise.all([
			prisma.episode.create({
				data: { podcast_id: pA.podcast_id, title: "A", audio_url: "x" },
			}),
			prisma.episode.create({
				data: { podcast_id: pB.podcast_id, title: "B", audio_url: "x" },
			}),
			prisma.episode.create({
				data: {
					podcast_id: pB.podcast_id,
