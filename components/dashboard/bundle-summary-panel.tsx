 ...
 */
export function BundleSummaryPanel({ userCurationProfile, subscription, onUpdateBundle }: BundleSummaryPanelProps) {
	const { sections } = dashboardCopy;

	const hasValidBundle = userCurationProfile?.is_bundle_selection && userCurationProfile?.selectedBundle;
	if (!hasValidBundle) {
		return null;
	}
	...
	return (
		<div className="rounded-xl w-screen md:w-full pt-0 md:pr-0 md:mr-0 flex flex-col lg:flex-col lg:justify-center lg:items-center md:p-8 sm:pt-0 md:pt-6 max-w-[87vw] p-0 m-0 mx-auto md:mx-0 md:overflow-hidden lg:max-w-[249px] lg:gap-12 overflow-hidden mt-8 mb-4 md:mt-0">
			<div className="px-0 lg:gap-2 md:w-full flex flex-col md:justify-between md:p-0 md:rounded-none">
				<h2 className="pt-8 font-bold md:text-secondary-foreground md:pt-0 md:px-0 pb-3 text-sm max-w-[100%] hidden md:block">{sections.bundleFeed.title}</h2>
				<div className="md:rounded-2xl">
					<div className="md:border-1 bg-sidebar shadow-indigo-800/90 md:shadow-none border border-border mx-auto py-6 md:pt-4 md:pb-0 px-5 w-full h-fit shadow-lg md:rounded-t-lg text-secondary-foreground md:text-primary-foreground">
						<Button className="inline-flex text-sm md:text-xs justify-end w-full px-0 mb-2 text-foreground" variant="ghost" size="xs" onClick={onUpdateBundle} aria-label="Update bundle selection">
							<Edit className="text-ring" />
							{sections.bundleFeed.updateButton}
						</Button>
						<div className="mb-4 flex flex-col">
							<p className="text-xs w-full uppercase font-sans font-bold text-primary-foreground-muted md:text-secondary-foreground-muted md:not-italic p-0 mb-2">
							...
							<div className="bg-card flex w-full py-8 px-3 md:px-2 md:py-1 md:rounded-sm border-1 border-sidebar-border/50 shadow-sm shadow-indigo-950/90 rounded-md gap-3">
								<Typography className="flex w-full font-bold items-center gap-2">
									...
									<span className="text-base text-foreground gap-3 font-sans text-left font-bold">{userCurationProfile.selectedBundle?.name}</span>
								</Typography>
							</div>
						</div>
					</div>
					...
					<div className="hidden md:inline mt-0 w-full">
						<div className="bg-[#393247]/30 border-t-0 overflow-hidden rounded-b-2xl border-1 border-[#51516500] px-4 p-4">
							<Body className="pt-0 pl-2 text-foreground/90 uppercase font-bold font-sans text-[10px]">{sections.bundleFeed.summaryLabel}</Body>
							<div className="flex flex-col justify-start gap-2 items-start my-2 px-0 w-full border-1 border-gray-800/30 rounded-md overflow-hidden pt-0">
								<div className="flex flex-row justify-between gap-1 items-center h-9 w-full bg-black/10 py-3 px-2">
									<span className="font-sans text-blue-200 text-xs">{sections.bundleFeed.bundledEpisodesLabel}</span>
								...
								<div className="flex flex-row justify-between gap-2 items-center h-5 w-full py-3 px-2">
									<span className="text-primary text-xs font-medium font-sans">{sections.bundleFeed.planLabel}</span>
									<span className="uppercase text-[0.7rem] w-full left text-indigo-200/80 font-sans">{getPlanDisplay()}</span>
								</div>
							</div>
						</div>
					</div>
