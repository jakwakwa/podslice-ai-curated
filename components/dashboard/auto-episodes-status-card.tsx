...
import { CheckCircle2, Clock, Sparkles, Video } from "lucide-react";
...
export async function AutoEpisodesStatusCard() {
	...
	// Next scheduled run is 6:00 PM UTC daily
	const now = new Date();
	...
	const minutesUntilNextRun = Math.floor(
		(timeUntilNextRun % (1000 * 60 * 60)) / (1000 * 60)
	);

	return (
		<div className="bg-bigcard w-full flex flex-col gap-0 justify-start mb-0 items-start shadow-xl shadow-indigo/30 mt-0 md:m-0 xl:flex-col md:gap-4 py-8 p-1 md:mt-4 md:mb-0 outline-1 outline-primary md:rounded-3xl lg:pb-8 lg:pt-0 overflow-hidden md:p-0 md:justify-start align-start lg:mb-0">
					...
					<>
						...
						<div className="bg-sidebar/50 p-4 rounded-lg border border-border/50">
							<div className="space-y-1">
								<p className="text-xs text-primary-foreground/60 uppercase tracking-wide font-mono font-bold">
						...
						{latestAutoEpisode ? (
							<div className="space-y-2 pt-2 border-t border-border/50">
								<div className="flex items-center gap-2 text-sm text-primary-foreground/60">
									...
									<span className="uppercase tracking-wide font-mono font-bold text-xs">
										Latest Episode
									</span>
								</div>
								<Link
									...
									<div className="flex items-center gap-3 mt-2">
										<Badge
											variant={
												latestAutoEpisode.status === "COMPLETED" ? "default" : "secondary"
											}
											className={
							...
							<div className="text-sm text-muted-foreground pt-2 border-t border-border/50">
								<p>
									No episodes generated yet. Your first episode will be created at the
									next scheduled run.
								</p>
							</div>
						)}
						...
						<div className="text-xs text-muted-foreground pt-2 border-t border-border/50 space-y-2 bg-sidebar/30 p-3 rounded-lg">
							<p className="font-bold text-primary-foreground/60 uppercase tracking-wide font-mono text-[0.65rem]">
								How it works:
							</p>
							<ul className="list-disc list-inside space-y-1 ml-2 text-xs">
								<li>New videos are fetched daily at 5:00 PM UTC</li>
								<li>The latest video is auto-processed at 6:00 PM UTC</li>
