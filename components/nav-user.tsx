      ...
      <SidebarMenuItem>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <SidebarMenuButton className="bg-white/10 h-9 w-22 md:h-14 md:w-full md:px-2 md:ml-0 m-0 p-0 rounded-xl overflow-hidden border-sidebar-border border-1  outline-0">
              <Avatar className=" h-10 w-11 object-cover flex items-center justify-center rounded-lg filter p-0 md:h-10 md:w-10">
                <AvatarImage src={user.avatar} alt={user.name} />
                <AvatarFallback className="text-foreground rounded-none hidden overflow-hidden">
              ...
              {!isMobile ? (
                <>
                  <div className="flex flex-col my-2   flex-1 text-left text-sm leading-tight ">
                    <span className="truncate font-medium text-sidebar-primary text-sm">
                      {user.name}
                    </span>
                    <span className="truncate text-muted text-xs font-[300] mt-1">
                      {user.email}
                    </span>
                  </div>
                  <MoreVerticalIcon className="text-sidebar-primary/80  md:block m-auto h-4 mr-3 w-0" />
                </>
              ) : null}
            </SidebarMenuButton>
          </DropdownMenuTrigger>
            ...
            <DropdownMenuLabel className="p-0 font-normal">
              <div className="flex items-center gap-2 px-1 md:px-2 py-1 text-left text-sm  text-foreground font-[400] leading-tight">
                ...
                <div className="flex flex-col gap-1 items-start justify-center flex-1 text-left text-xs leading-tight py-1 my-0">
                  ...
                  <span className="truncate my-0 font-[200] text-xs text-white">
                    {user.email}
                  </span>
                </div>
              </div>
