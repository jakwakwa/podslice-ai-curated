	...
	return (
		<div className="mt-2 flex flex-col px-8">
			{loading ? (
				...
				<>
					<div className={cn("text-[40px] leading-[46px] tracking-[-1.6px] font-medium")}>
...
export function PricingPlans({
	...
	const [frequency, _setFrequency] = useState<IBillingFrequency>({
		value: "month",
		label: "Monthly",
	...
	useEffect(() => {
		...
		if (!clientToken) {
			console.error(
				"Paddle Client Token is not set. Please ensure the NEXT_PUBLIC_PADDLE_CLIENT_TOKEN environment variable is configured correctly."
			);
			return;
		}
		...
		<>
			{!hasActiveSubscription &&
				paddleProductPlan?.map(tier => (
					<Card
						key={tier.priceId}
						className={cn("rounded-lg bg-card  overflow-hidden flex-1")}>
						<div
							className={cn(
