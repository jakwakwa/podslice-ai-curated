...
export function BundleList({ onBundleSelect }: BundleListProps) {
  ...
  const fetchCuratedBundles = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to load PODSLICE Bundles. Server responded with status ${response.status}.`,
        );
      }
      ...
      setError(
        error instanceof Error
          ? error.message
          : "An unexpected error occurred while loading PODSLICE Bundles.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  if (isLoading) {
    return (
      <div className="wrapper">
        <PageHeader
          title="PODSLICE Bundles"
          description="Choose from our pre-curated podcast bundles. Each bundle contains 5 carefully selected shows and cannot be modified once selected."
        />
  ...
  if (curatedBundles.length === 0) {
    return (
      <div className="max-w-2xl mx-auto">
        <p>
          No PODSLICE Bundles Available. Please check back later or contact
          ...
          >
            <CardContent className="p-2">
              <div className="flex flex-col">
                ...
                <div className="flex flex-col w-full min-w-0">
                  <div className="flex flex-row w-full min-w-0 rounded-lg gap-1">
                    ...
                    <div className="flex flex-col w-full min-w-0 py-4 items-start">
                      ...
                      <CardTitle className="text-lg font-semibold mb-2 truncate w-full">
                        {bundle.name}
                      </CardTitle>
                ...
                {/* Podcasts section */}
                <div className="flex flex-col rounded-lg w-full min-w-0 border border-border">
                  {/* Podcasts list */}
                  <div className="mb-0 p-4">
                    <h5 className="font-medium mb-1 text-body">
                      Included Podcasts:
                    </h5>
                    <ul className="space-y-1 pt-2">
                      ...
                      {(!bundle.podcasts || bundle.podcasts.length === 0) && (
                        <li className="text-sm text-muted-foreground">
                          No podcasts linked yet
                        </li>
                      )}
                    </ul>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-sm text-muted-foreground p-4">
                    ...
                    <span className="truncate">
                      {disabled
                        ? bundle.lockReason || "Requires higher plan"
                        : "Fixed Selection"}
                    </span>
                  </div>
                </div>
