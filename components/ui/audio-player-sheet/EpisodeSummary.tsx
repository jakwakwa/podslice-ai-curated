...
export const EpisodeSummary: FC<EpisodeSummaryProps> = ({ episode, isExpanded }) => {
	const [isTranscriptExpanded, _setIsTranscriptExpanded] = useState<boolean>(
		Boolean(isExpanded)
	);
	...
	const rawSummaryOrDescription = useMemo(() => {
		...
		if ("description" in episode && episode.description)
	...
	const normalizedSummary = useMemo(
		() =>
			rawSummaryOrDescription ? normalizeSummaryMarkdown(rawSummaryOrDescription) : null,
		[rawSummaryOrDescription]
	);
	...
	return (
		<div className="bg-[#16172262] backdrop-blur-md rounded-none">
			...
			<AnimatePresence initial={false}>
				...
				(normalizedSummary || ("transcript" in episode && episode.transcript)) ? (
					<motion.div
						key="transcript"
...
export const EpisodeHeader: FC<EpisodeHeaderProps> = ({
	episode,
	isTranscriptExpanded,
	onToggleTranscript,
}) => {
...
export const EpisodeTitle: FC<EpisodeTitleProps> = ({ episode }) => {
	...
	return (
		<h2 className="line-clamp-2 text-[18.64px] font-bold leading-[1.5] tracking-[0.009375em] text-slate-300 text-center px-6 mt-4 text-shadow-lg text-shadow-black/10 capitalize">
			{title}
		</h2>
	);
};
...
export const EpisodeSubtitle: FC<EpisodeSubtitleProps> = ({
	episode,
	podcastName,
	youtubeChannelName,
	isChannelLoading,
	...
	const subtitle = getEpisodeSubtitle(episode, {
		podcastName,
		youtubeChannelName,
		isChannelLoading,
	});
	...
	return (
		<p className="text-[13.69px]   leading-[1.72857] mt-1 tracking-[0.05142em] uppercase text-[#a484da] text-center text-shadow-md text-shadow-black/20">
			{subtitle}
		</p>
	);
};
