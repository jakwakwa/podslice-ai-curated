...
const PageHeader = React.forwardRef<HTMLDivElement, PageHeaderProps>(({ title, description, level = 1, button }, _ref) => {
	...
	return (
		<div className="my-0 pt-12 pb-9 md:pt-4 md:px-8  h-fit lg:pt-12 lg:pb-12 shadow-sm  rounded-none  outline-none w-screen bg-[#bddcfc82] md:bg-secondary  relative border-b-none  md:rounded-b-3xl md:border-l-10 md:border-border  md:rounded-3xl p-6 flex flex-col  shadow-slate-700/30 md:shadow-lg justify-between gap-8 md:w-full mb-0 md:mb-3 md:flex-row lg:rounded-3xl    md:shadow-slate-900/20 md:items-center md:justify-between ">
			<div className="flex flex-col justify-center w-full gap-3">



				<h2 className="flex flex-col w-full text-xl  lg:text-2xl font-bold px-0 md:px-0 pt-0 pb-0 md:py-0  text-secondary-foreground leading-[1.5] max-w-screen lg:max-w-2xl lg:py-1 capitalize ">{title}</h2>
				{description && (
					<Typography
						as="p"
						variant="body"
						className="text-base font-medium md:text-[1rem] px-0  md:px-0  md:py-1  text-secondary-foreground-muted leading-[1.4] max-w-screen  w-full md:max-w-[97%] lg:text-base  lg:max-w-2xl">
						{description}
					</Typography>
				)}
			</div>
