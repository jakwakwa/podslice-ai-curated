...
const PageHeader = React.forwardRef<HTMLDivElement, PageHeaderProps>(({ title, description, level = 1, button }, _ref) => {
	...
	return (
		<div className="my-0 pt-8 md:pt-4 backdrop-blur-lg bg-accent border border-border relative md:border-l-10 rounded-md p-6 flex flex-col  justify-between gap-8 w-full mb-0 md:mb-3 md:px-4 md:flex-row lg:rounded-3xl  shadow-black/40 shadow-lg md:justify-between ">
			<div className="flex flex-col justify-center w-full">



				<h2 className="flex flex-col w-full  text-xl font-bold px-0 md:px-0 pt-0 pb-0 md:py-0 text-shadow-md text-primary-foreground leading-[1.5] max-w-screen lg:max-w-2xl lg:text-xl lg:py-2 lg:uppercase ">{title}</h2>
				{description && (
					<Typography
						as="p"
						variant="body"
						className="text-sm md:text-[0.9rem] px-0  md:px-0  md:py-1  text-primary-foreground/70 leading-[1.4] max-w-screen  text-shadow-sm font-normal w-full md:max-w-[97%] lg:text-base  lg:max-w-2xl">
						{description}
					</Typography>
				)}
			</div>
