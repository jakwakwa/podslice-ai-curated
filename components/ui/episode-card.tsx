...
export function EpisodeCard({
	...
	const { channelImage: youtubeChannelImage, isLoading: isChannelLoading } =
		...
		<Card
			...
			<div className="w-full h-fit flex flex-col-reverse sm:flex-row gap-8 justify-between px-0  md:flex-col-reverse lg:flex-row sm:gap-5 lg:gap-8 items-evenly   py-0 lg:py-1 lg:px-0 lg:justify-betwen lg:items-between relative ">
				{(() => {
					...
					if (isNewsEpisode) {
						return (
							<div className="aspect-video w-full h-fit max-h-[110px] max-w-[150px]   xl:h-42 md:h-32 sm:max-h-[110px] sm:max-w-[140px] lg:h-auto lg:max-h-[90px] md:hidden xl:flex shadow-[1px_2px_5px_2px_#003e3cca,1px_-7px_19px_0px_#013e3cca_inset] bg-[#3626efc7] shadow-black/35 border-[#421b8bed] rounded-md md:rounded-lg overflow-hidden border-2	 outline-1 outline-gray-500 lg:min-w-[160px]">
								<Image
							...
							return (
								<div className="aspect-video w-full h-fit max-h-[110px] max-w-[150px]   xl:h-42 md:h-32 sm:max-h-[110px] sm:max-w-[140px] lg:h-auto lg:max-h-[90px] lg:hidden xl:flex shadow-[1px_2px_5px_2px_#003e3cca,1px_-7px_19px_0px_#013e3cca_inset] bg-cyan-900 shadow-black/35 border-[#421b8bed] rounded-md md:rounded-lg overflow-hidden border-2	 outline-1 outline-gray-500   lg:min-w-[160px]">
									<Image
										...
										alt={title}
										className="h-full w-full inline-flex content-center object-cover opacity-80  shadow-zinc-800"
										width={300}
										height={200}
									/>
								</div>
				...
				<div className="flex w-[90%] flex-col justify-between h-fit items-start content-start py-0 md:px-3 z-10 md:flex-col  md:pl-0  md:gap-0 md:justify-between md:min-h-fit lg:pl-2 lg:pt-2">
					{detailsHref ? (
						<Link
							className="text-[1.1rem] font-semibold md:w-full md:text-[0.65]  leading-tight line-clamp-2 h-auto max-h-14 lg:max-w-[85%] lg:text-base mb-5 lg:pb-0 text-primary-foreground text-shadow-2xs hover:text-cyan-200 hover:decoration-azure-800 hover:opacity-90 transition-all duration-300 text-left capitalize"
							href={detailsHref}>
							<div>
								{title}
								<br />
								<span className="text-[0.6rem] tracking-wider font-light uppercase pr-1">
									view details
								</span>
							</div>
						</Link>
