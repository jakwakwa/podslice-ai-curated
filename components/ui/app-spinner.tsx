...
export function AppSpinner({
			...
			case "gradient":
				return (
					<div className={cn("relative", baseClasses)}>
						<div className=" border-2 border-transparent rounded-full animate-spin" />
						<div className="absolute inset-0.5 bg-background/20  rounded-full h-12 w-12" />
					</div>
				);
	...
	return (
		<div className={cn("flex flex-col items-start justify-start gap-3", className)}>
			...
			{label && (
				<span className={cn("text-xs text-left", getLabelColorClass(labelColor))}>
					{label}
				</span>
			)}
		</div>
