		...
		(set, get) => ({
			...
			createUserCurationProfile: async data => {
				...
				try {
					...
					if (!response.ok) {
						...
						throw new Error(
							errorData.message || "Failed to create user curation profile"
						);
					}

					const userCurationProfile = await response.json();
					set(
						{ userCurationProfile, isLoading: false },
						false,
						"createUserCurationProfile:success"
					);
					...
					set(
						...
						false,
						"createUserCurationProfile:error"
					);
			...
			updateUserCurationProfile: async data => {
				...
				if (!data.profile_id) {
					set(
						{
							error: "User Curation Profile ID is required for update",
							isLoading: false,
						},
						false,
						"updateUserCurationProfile:idError"
					);
				...
				try {
					...
					if (!response.ok) {
						const errorData = await response.json();
						throw new Error(
							errorData.message || "Failed to update user curation profile"
						);
					}

					const updatedUserCurationProfile = await response.json();
					set(
						{ userCurationProfile: updatedUserCurationProfile, isLoading: false },
						false,
					...
					set(
						{ error: message, isLoading: false },
						false,
						"updateUserCurationProfile:error"
					);
			...
			deactivateUserCurationProfile: async () => {
				set(
					...
					false,
					"deactivateUserCurationProfile:start"
				);

				if (!get().userCurationProfile?.profile_id) {
					set(
						{ error: "No active user curation profile to deactivate", isLoading: false },
						false,
						"deactivateUserCurationProfile:noProfile"
					);
				...
				try {
					const response = await fetch(
						`/api/user-curation-profiles/${get().userCurationProfile!.profile_id}`,
						{
							method: "DELETE",
						}
					);
					...
					if (!response.ok) {
						const errorData = await response.json();
						throw new Error(
							errorData.message || "Failed to deactivate user curation profile"
						);
					}

					set(
						{ userCurationProfile: null, isLoading: false },
						false,
						"deactivateUserCurationProfile:success"
					);
					...
					set(
						{ error: message, isLoading: false },
						false,
						"deactivateUserCurationProfile:error"
					);
