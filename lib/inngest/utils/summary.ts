...
export interface FinancialResponse {
	...
	writtenContent: {
		executiveBrief: string; // "The CEO discussed..."
		investmentImplications: string; // "This implies a long position on..."
	};
	audioScript: string; // Conversational style
}
...
Your task is to analyze the provided transcript and extract structured financial intelligence, write an executive brief, and generate a conversational audio script.
...
OUTPUT SCHEMA (JSON ONLY):
{
  "structuredData": {
    ...
    "sentimentLabel": "BULLISH" | "NEUTRAL" | "BEARISH",
    "tickers": string[] (e.g., ["$BTC", "$NVDA"])
  },
  "writtenContent": {
    "executiveBrief": string (Focus on CAPEX, ROI, and Moats. Markdown format allowed.),
    "investmentImplications": string (Actionable insights. Markdown format allowed.)
  },
  "audioScript": string (Written for listening, not reading. Conversational style. 1000-1500 words.)
}

RULES:
1. Executive Brief: Focus on CAPEX, ROI, and Moats.
2. Audio Script: Must be conversational, engaging, and suitable for TTS. DO NOT include "Host:" or "Speaker:" prefixes, just the spoken text for a single narrator.
3. Tickers: Extract ALL valid crypto/stock tickers. Prefix with $.
4. Sentiment: -1.0 is extreme panic, 1.0 is extreme greed.
`;
