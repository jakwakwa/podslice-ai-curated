...
export const enqueueTranscriptionJob = inngest.createFunction(
	...
	async ({ event, step }) => {
		...
		await step.run("enrich-youtube-metadata", async () => {
			try {
				...
				const existing = await prisma.userEpisode.findUnique({
					where: { episode_id: userEpisodeId },
					select: { episode_title: true },
				});
				const currentTitle = existing?.episode_title?.trim() || "";
				const shouldReplace =
					currentTitle.length < 4 || /^(untitled|video)$/i.test(currentTitle);
				...
				await writeEpisodeDebugLog(userEpisodeId, {
					...
					status: "success",
					meta: { fetched: true, replacedTitle: shouldReplace },
				});
			} catch (_err) {
				const errorMessage =
					_err instanceof Error ? _err.message : "metadata fetch error";
		...
		await writeEpisodeDebugLog(userEpisodeId, {
			step: "saga",
			status: "info",
			message: "sending transcription.job.requested",
			meta: { jobId },
		});
		...
		await writeEpisodeDebugLog(userEpisodeId, {
			step: "saga",
			status: "success",
			message: "sent transcription.job.requested",
			meta: { jobId },
		});
