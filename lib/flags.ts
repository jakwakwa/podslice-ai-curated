...
function readFlagValue(flagName: string): string | undefined {
	...
	const rawPublic = process.env[publicName as keyof NodeJS.ProcessEnv] as unknown as
		| string
		| undefined;
	if (typeof rawPublic === "string" && rawPublic.length > 0) return rawPublic;
	const raw = process.env[flagName as keyof NodeJS.ProcessEnv] as unknown as
		| string
		| undefined;
	return raw;
}
...
export function isEnabled(flagName: string, defaultValue: boolean = false): boolean {
	...
	if (TRUE_VALUES.has(normalized as typeof TRUE_VALUES extends Set<infer T> ? T : never))
		return true;
	if (
		FALSE_VALUES.has(normalized as typeof FALSE_VALUES extends Set<infer T> ? T : never)
	)
		return false;
	return defaultValue;
}
 ...
 */
export function getAdminPanelsFlags(defaults?: {
	bundles?: boolean;
	podcasts?: boolean;
	episodes?: boolean;
}) {
